<?php
//lokalizacja_panel.php - fragment panelu lokalizacji (dashboard-friendly, z paginacją i akcjami) związanych z poszczegółnymi magazynami
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        $del = $pdo->prepare("DELETE FROM lokalizacje WHERE id = :id");
        $del->execute([':id' => $deleteId]);

        echo json_encode(['success' => true, 'message' => 'Usunięto lokalizację: ' . ($item['nazwa'] ?: $item['nazwa'])]);
        exit;
    } catch (Throwable $e) {
        error_log('lokalizacja_panel delete error: ' . $e->getMessage());
...
try {
    $totalStmt = $pdo->query("SELECT COUNT(*) AS total FROM lokalizacje");
    $totalRecords = (int)$totalStmt->fetchColumn();
} catch (Throwable $e) {
    error_log('lokalizacja_panel count error: ' . $e->getMessage());
    $totalRecords = 0;
}
...
try {
    $stmtp = $pdo->prepare("
        SELECT 
          ...
          FROM lokalizacje l
          LEFT JOIN magazyny m ON l.id_mag = m.id
        ORDER BY nazwa ASC
    ...
    error_log('lokalizacja_panel fetch error: ' . $e->getMessage());
    $rowsp = [];
}

/* Helper URL paginacji */
function pageUrlUsers($p) {
    return 'lokalizacja_panel.php?' . http_build_query(['page' => $p]);
}
...
<div id="users-panel" class="content">
  ...
  <p>Lista lokalizacji sprzętu w systemie. Możesz dodawać, edytować, lub usunąć wybraną lokalizację.</p>

  <div class="actions actions--toolbar" role="toolbar" aria-label="Akcje użytkowników" style="margin-bottom:10px;">
    <button id="addUserBtn" class="btn" type="button" title="Dodaj nowy podmiot">＋ Dodaj lokalizację</button>
  </div>

  <?php if (empty($rowsp)): ?>
    <p class="info-message">Brak lokalizacji.</p>
  <?php else: ?>
    <!-- Tabela ustawiona na szerokość 55% i wyśrodkowana -->
    <table class="tabela" role="table" aria-label="Lista kategorii" style="width:55%; margin:0 auto;">
      <thead>
        <tr>
          <th>L.p.</th>
          <th>Nazwa</th>
        ...
        <tr data-id="<?= (int)$r['id'] ?>">
          ...
          <td class="col-actions">
            <div class="akcje" role="group" aria-label="Akcje">
              ...
              <button type="button" class="btn-akcja edit open-modal" data-url="lokalizacja_edit.php?id=<?= (int)$r['id'] ?>">Edytuj</button>
  ...
  window.initUsersPanel = function initUsersPanel() {
    ...
    function reloadPanel() {
      try {
        if (typeof window.loadContent === 'function') {
          var url = window.currentContentUrl || 'lokalizacja_panel.php?page=<?= $page ?>';
          window.loadContent(url);
          return;
        }
      } catch (e) { console.error(e); }
      window.location.href = 'lokalizacja_panel.php?page=<?= $page ?>';
    }

    if (addBtn) {
      addBtn.addEventListener('click', function(e) {
        e.preventDefault();
        if (typeof window.openModal === 'function') window.openModal('lokalizacja_add.php');
        else window.location.href = 'lokalizacja_add.php';
      });
    }
    ...
    panel.addEventListener('click', function(e) {
      ...
      if (del) {
        ...
        if (!confirm('Czy na pewno chcesz usunąć lokalizację: ' + name + ' ?')) return;
