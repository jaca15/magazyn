<?php
// podmiot_panel.php - fragment panelu podmiotów (dashboard-friendly, z paginacją i akcjami)
...
*/
    try {
        ...
        echo json_encode(['success' => true, 'message' => 'Usunięto podmiot: ' . ($item['nazwa_pelna'] ?: $item['nazwa_skrocona'])]);
        exit;
    } catch (Throwable $e) {
        error_log('podmiot_panel delete error: ' . $e->getMessage());
...
try {
    ...
    error_log('podmiot_panel count error: ' . $e->getMessage());
    $totalRecords = 0;
}
...
function pageUrlUsers($p) {
    return 'podmiot_panel.php?' . http_build_query(['page' => $p]);
}
...
<div id="users-panel" class="content">
  ...
  <p>Lista podmiotów w systemie. Możesz dodawać, edytować, lub usunąć podmiot.</p>

  <div class="actions actions--toolbar" role="toolbar" aria-label="Akcje użytkowników" style="margin-bottom:10px;">
    <button id="addUserBtn" class="btn" type="button" title="Dodaj nowy podmiot">＋ Dodaj podmiot</button>
  </div>

  <?php if (empty($rowsp)): ?>
    <p class="info-message">Brak podmiotów.</p>
  <?php else: ?>
    <!-- Tabela ustawiona na szerokość 65% i wyśrodkowana -->
    <table class="tabela" role="table" aria-label="Lista podmiotów" style="width:75%; margin:0 auto;">
      <thead>
        <tr>
          <th>L.p.</th>
          <th>Nazwa pełna</th>
      ...
      <tbody>
        <?php foreach ($rowsp as $idx => $r):
          $displayName = $r['nazwa_pelna'] ?: $r['nazwa_skrocona'];
        ?>
        <tr data-id="<?= (int)$r['id'] ?>">
          <td><?= h(($page - 1) * $perPage + $idx + 1) ?></td>
          <td title="<?= h($r['nazwa_pelna']) ?>"><?= h($r['nazwa_pelna']) ?></td>
          <td title="<?= h($r['nazwa_skrocona']) ?>"><?= h($r['nazwa_skrocona']) ?></td>
          <td><?= h($r['adres'] ?? '') ?></td>
          <td><?= h($r['telefon'] ?? '') ?></td>
          <td><?= h($r['email'] ?? '') ?></td>
          <td><?= h($r['uwagi'] ?? '') ?></td>
          ...
          <td class="col-actions">
            <div class="akcje" role="group" aria-label="Akcje">
              ...
              <button type="button" class="btn-akcja edit open-modal" data-url="podmiot_edit.php?id=<?= (int)$r['id'] ?>">Edytuj</button>
    ...
    function reloadPanel() {
      try {
        if (typeof window.loadContent === 'function') {
          var url = window.currentContentUrl || 'podmiot_panel.php?page=<?= $page ?>';
          window.loadContent(url);
          return;
        }
      } catch (e) { console.error(e); }
      window.location.href = 'podmiot_panel.php?page=<?= $page ?>';
    }

    if (addBtn) {
      addBtn.addEventListener('click', function(e) {
        e.preventDefault();
        if (typeof window.openModal === 'function') window.openModal('podmiot_add.php');
        else window.location.href = 'podmiot_add.php';
      });
    }
