<?php
//magazyn_panel.php - fragment panelu magazynów (dashboard-friendly, z paginacją i akcjami)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        $del = $pdo->prepare("DELETE FROM magazyny WHERE id = :id");
        $del->execute([':id' => $deleteId]);

        echo json_encode(['success' => true, 'message' => 'Usunięto magazyn: ' . ($item['nazwa'] ?: $item['nazwa'])]);
        exit;
    } catch (Throwable $e) {
        error_log('magazyn_panel delete error: ' . $e->getMessage());
...
try {
    ...
    error_log('magazyn_panel fetch error: ' . $e->getMessage());
    $rowsp = [];
}

/* Helper URL paginacji */
function pageUrlUsers($p) {
    return 'magazyn_panel.php?' . http_build_query(['page' => $p]);
}
...
<div id="users-panel" class="content">
  ...
  <p>Lista Magazynów sprzętu w systemie. Możesz dodawać, edytować, lub usunąć wybrany magazyn.</p>

  <div class="actions actions--toolbar" role="toolbar" aria-label="Akcje użytkowników" style="margin-bottom:10px;">
    <button id="addUserBtn" class="btn" type="button" title="Dodaj nowy podmiot">＋ Dodaj magazyn</button>
  </div>

  <?php if (empty($rowsp)): ?>
    <p class="info-message">Brak magazynu.</p>
  <?php else: ?>
        ...
        <tr data-id="<?= (int)$r['id'] ?>">
          ...
          <td class="col-actions">
            <div class="akcje" role="group" aria-label="Akcje">
              ...
              <button type="button" class="btn-akcja edit open-modal" data-url="magazyn_edit.php?id=<?= (int)$r['id'] ?>">Edytuj</button>
  ...
  window.initUsersPanel = function initUsersPanel() {
    ...
    function reloadPanel() {
      try {
        if (typeof window.loadContent === 'function') {
          var url = window.currentContentUrl || 'magazyn_panel.php?page=<?= $page ?>';
          window.loadContent(url);
          return;
        }
      } catch (e) { console.error(e); }
      window.location.href = 'magazyn_panel.php?page=<?= $page ?>';
    }

    if (addBtn) {
      addBtn.addEventListener('click', function(e) {
        e.preventDefault();
        if (typeof window.openModal === 'function') window.openModal('magazyn_add.php');
        else window.location.href = 'magazyn_add.php';
      });
    }
    ...
    panel.addEventListener('click', function(e) {
      ...
      if (del) {
        ...
        if (!confirm('Czy na pewno chcesz usunąć magazyn: ' + name + ' ?')) return;
